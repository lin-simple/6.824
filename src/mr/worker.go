...
func Worker(mapf func(string, string) []KeyValue,
	...
	if succ == false {
		fmt.Println("Failed to get reduce task count, worker exits.")
		return
	}
	nReduce = n
	...
	for {
		...
		if succ == false {
			fmt.Println("Failed to contact master, worker exits.")
			return
		}
		if reply.TaskType == ExitTask {
			fmt.Println("All tasks are done, worker exits.")
			return
		}
		exit, succ := false, true
		if reply.TaskType == NoTask {
			// The mr job not done, but all map or reduce tasks are executing
		...
		if exit || !succ {
			fmt.Println("Master exited or all tasks are done, worker exits.")
			return
		}
